...
{
    if(!_Volume_EMA5) {
        _Volume_EMA5 = @((self.Volume + 2 * self.PreviousKlineModel.Volume_EMA5.floatValue)/3);
    }
    return _Volume_EMA5;
}
...
{
    if(!_Volume_EMA10) {
        _Volume_EMA10 = @((2*self.Volume + 9 * self.PreviousKlineModel.Volume_EMA10.floatValue)/11);
    }
...
- (NSNumber *)Volume_MA5
{
    if([Y_StockChartGlobalVariable isEMALine] == Y_StockChartTargetLineStatusMA)
    {
        if (!_Volume_MA5) {
            ...
            if (index >= 4) {
                if (index > 4) {
                    _Volume_MA5 = @((self.SumOfLastVolume.floatValue - self.ParentGroupModel.models[index - 5].SumOfLastVolume.floatValue) / 5);
                } else {
                    _Volume_MA5 = @(self.SumOfLastVolume.floatValue / 5);
                }
            }
        }
    } else {
...
- (NSNumber *)Volume_MA10
{
    if([Y_StockChartGlobalVariable isEMALine] == Y_StockChartTargetLineStatusMA)
    {
        if (!_Volume_MA10) {
            ...
            if (index >= 9) {
                if (index > 9) {
                    _Volume_MA10 = @((self.SumOfLastVolume.floatValue - self.ParentGroupModel.models[index - 10].SumOfLastVolume.floatValue) / 10);
                } else {
                    _Volume_MA10 = @(self.SumOfLastVolume.floatValue / 10);
                }
            }
        }
    } else {
...
- (void) initWithDict:(NSDictionary *)dict
{
    ...
    if (self)
    {
//        _Date = [NSString stringWithFormat:@"%@",dict[@"CloseTime"]];
...
//        self.SumOfLastVolume = @(_Volume + self.PreviousKlineModel.SumOfLastVolume.floatValue);
        ...
        _Date = [NSString stringWithFormat:@"%@",dict[@"E"]];
        _Open = @([dict[@"O"] floatValue]);
        _High = @([dict[@"H"] floatValue]);
        _Low = @([dict[@"L"] floatValue]);
        _Close = @([dict[@"C"] floatValue]);
        _Volume = [dict[@"V"] floatValue];
        self.SumOfLastClose = @(_Close.floatValue + self.PreviousKlineModel.SumOfLastClose.floatValue);
        self.SumOfLastVolume = @(_Volume + self.PreviousKlineModel.SumOfLastVolume.floatValue);
    }
}
