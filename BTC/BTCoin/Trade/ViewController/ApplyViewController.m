...
#define kApplySelectStoreActionButtonBeginTag 78280



@interface ApplyViewController ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate,SRWebSocketDelegate>{
...
@property (nonatomic,strong)UILabel * priceTitleLabel;//"价格"label
...
-(void)requestData
{
    NSString * urlStr = [NSString stringWithFormat:@"%@/lock/qryLockCurrentInfo.do",BaseHttpUrl];
    
    NSMutableDictionary * mDict = [NSMutableDictionary dictionaryWithCapacity:1];
    [mDict setValue:@([_selectedModel.fCoinType integerValue]) forKey:@"id"];
    
    [self showLoadingMBProgressHUD];
    
    __weak typeof(self) weakSelf = self;
    [SZHTTPSReqManager SZPostRequestWithUrlString:urlStr appendParameters:mDict bodyParameters:nil successBlock:^(id responseObject) {
        [weakSelf hideMBProgressHUD];
        BaseModel * base = [BaseModel modelWithJson:responseObject];
        if (!base.errorMessage) {
            @try {
                weakSelf.tips =  responseObject[@"data"][@"tremark"];
            }
            ...
            if (weakSelf.isNeedShowTips) {
                [weakSelf showTips];
                weakSelf.isNeedShowTips = NO;
            }
        }else{
...
-(void)getMarketDataNotifaction:(NSNotification *)noti
{
    ...
    if ([_selectedModel.fCoinType integerValue] == stockId) {
        ...
        [self.topView refreshViewWithModel:_selectedModel];
        self.priceTF.text = _selectedModel.fNewDealPrice;
    }
}
...
-(void)selectStoreAction:(UIButton *)button{
    for (NSInteger i = 0; i< 4; i++) {
        UIButton * but =  [self.leftSuperView viewWithTag:(kApplySelectStoreActionButtonBeginTag + i)];
        if (but == button) {
            but.selected = YES;
...
-(void)showTips{
    if (!_tips) {
        self.isNeedShowTips = YES;
        [self requestData];
        return;
    }
    
    ApplyNoticeView * applyView = [[ApplyNoticeView alloc]init];
    [applyView setText:_tips];
    
    __weak typeof(self)weakSelf = self;
    applyView.goNextBlock = ^{
//        [weakSelf inputTradePassWord];
...
-(void)packParametersWithPWD:(NSString * )passWord
{
    ...
    [mDict setValue:_selectedModel.fCoinType forKey:@"ftrademapping"];
    [mDict setValue:[AppUtil md5:passWord]forKey:@"tradePwd"];
    [mDict setValue:_priceTF.text forKey:@"price"];
    [mDict setValue: _numberTF.text forKey:@"count"];
    [mDict setValue:@(10) forKey:@"buyType"];
...
//    [self.btnLeft removeFromSuperview];
    [self setTitleText:NSLocalizedString(@"申购", nil)];

    _topView = [[TradeCoinMessageView alloc]init];
    [self.view addSubview:_topView];
    [_topView disableTapEvent];
    ...
    _priceTitleLabel = [self createLabelWithTextColor:UIColorFromRGB(0x333333) font:[UIFont fontWithName:@"PingFang-SC-Medium" size:16]];
    _priceTitleLabel.text = NSLocalizedString(@"申购价格", nil);
    [_leftSuperView addSubview:_priceTitleLabel];
    ...
    _priceTF.backgroundColor = UIColorFromRGB(0xF5F6FA);
    _priceTF.text = _selectedModel.fNewDealPrice;
    _priceTF.enabled = NO;
    [_priceTF mas_makeConstraints:^(MASConstraintMaker *make) {
        make.leading.mas_equalTo(16);
        make.trailing.mas_equalTo(0);
        make.top.mas_equalTo(_priceTitleLabel.mas_bottom).offset(10);
        make.height.mas_equalTo(43);
    }];
    
    _numberTF = [self createTextFieldWithPlaceholder:@"申购量"];
   ...
   CGFloat buttonW = leftW /4.0 - 15;
    ...
    for (int i = 0; i < 4; i++) {
        ...
        button.tag = kApplySelectStoreActionButtonBeginTag + i;
    ...
    _buyOrSelButton.backgroundColor =  UIColorFromRGB(0x03c087);
    [_buyOrSelButton setTitle:NSLocalizedString(@"申购", nil) forState:UIControlStateNormal];
