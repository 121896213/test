...
#define kScale (kScreenWidth /414)
...
#pragma mark 小鸟加载图像
/**显示小鸟加载*/
#define Loading_Pig_Show(view) [view makeToastActivity_pig]
/**隐藏加载*/
#define Loading_Hide(view) [view hideToastActivity]
/**显示杯子的加载*/
#define Loading_Gold_Show(view) [view makeToastActivity_gold]
...
typedef void(^CommonIntegerBlock)(NSInteger returnValue);
typedef void(^CommonVoidBlock)(void);
typedef void(^CommonImageBlock)(UIImage *image);
typedef void(^CommonBoolBlock)(BOOL boolValue);
typedef void(^CommonStringBlock)(NSString *stringValue);
typedef void(^CommonDictionaryBlock)(NSDictionary *param);
typedef void(^CommonArrayBlock)(NSArray *param);
typedef void(^CommonDataBlock)(NSData *data);
typedef void(^CommonStructBlock)(char *pHeader, char *result);
typedef void(^CommonIndexPathBlock)(NSIndexPath *indexPath);

typedef void(^CommonDataWithTypeBlock)(NSData *data, NSInteger type);

typedef void(^CommonDoubleArrayBlock)(NSArray *param1, NSArray *param2);

#define  LISTENTOAPPLOGINNOTIFICATION   @"listenToAppLoginNotification"
