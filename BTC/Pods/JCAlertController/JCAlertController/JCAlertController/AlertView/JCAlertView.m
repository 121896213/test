...
- (void)willMoveToSuperview:(UIView *)newSuperview {
    ...
    if (self.message.length > 0) {
        NSAttributedString *contentStr = [[NSAttributedString alloc] initWithString:self.message attributes: [self contentViewStyle]];
    ...
    if (titleHeight + contentHeight + self.buttonHeight < style.alertView.maxHeight) { // in max height
        ...
        if (contentHeight > 0) {
            if (contentSize.height <= contentCharHeight) {
                ...
                contentView.attributedText=[self contentViewAttributeString];
                ...
                if (contentView.frame.size.height < contentView.contentSize.height) {
                    ...
                    self.frame = CGRectMake(0, 0, style.alertView.width, titleHeight + newF.size.height + self.buttonHeight);
...
-(NSDictionary*)contentViewStyle{
    
    
    NSMutableParagraphStyle *paraStyle = [[NSMutableParagraphStyle alloc] init];
    paraStyle.lineBreakMode = NSLineBreakByCharWrapping;
    paraStyle.alignment = self.style.content.textAlignment;
    ...
    NSDictionary *dic = @{NSFontAttributeName:self.style.content.font, NSParagraphStyleAttributeName:paraStyle, NSKernAttributeName:@0.5f
                          };
    
    return dic;
}
-(NSAttributedString*)contentViewAttributeString{
    
    return [[NSAttributedString alloc] initWithString:self.message attributes:[self contentViewStyle]];;
}
